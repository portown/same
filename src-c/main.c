...
static BOOL InitApp(HINSTANCE hInst)
{
    WNDCLASSEX wc;
...
static BOOL InitInstance(HINSTANCE hInst, int nCmd)
{
    HWND hWnd;
    SIZE szWnd;
    ...
    switch (msg)
    {
        ...
        case WM_LBUTTONUP:
            ...
            switch (ucRet)
            {
                ...
                case CR_BEGINMASK4:
                    destroyGameScene(scene);
                    scene = createGameScene(GSK_GAME, createGameSceneGame(GAMEX, GAMEY, ucRet - CR_BEGINNORMAL, (unsigned long)time(NULL)));
                    break;
